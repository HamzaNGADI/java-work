...
public class DicoListeTriee implements Dictionnaire {
	...
	public Dictionnaire addWords(String... words) {
        for(int i=0;i<words.length;i++)
        {
        	...
        	if(p == size() || liste.get(p).compareTo(words[i])!=0) liste.add(p,words[i]);
        }
		return this;
	}
	...
	public Dictionnaire cutWords(String... words) {
        ...
        {
        	int p =position(words[i]);
        	if((p >=0 && p<size()) && liste.get(p).compareTo(words[i])==0) {
        		liste.remove(p); 
        	}
        }
		return this; 
	...
	public boolean exists(String word) {
        int p = position(word);
        if((p >=0 && p<size()) && liste.get(p).compareTo(word)==0) return true;
        return false;
	}
